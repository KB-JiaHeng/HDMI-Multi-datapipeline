...
def run_icp_tracking(model_pcd, obj_pts_dict, obj_norms_dict, total_frames,
    ...
    for fidx in range(total_frames):
        ...
        if len(target.points) < min_points:
            if fidx > 0 and poses_list[fidx - 1] is not None:
                poses_list[fidx] = poses_list[fidx - 1]
            continue
        ...
        if need_init:
            ...
            T_result, fit = full_rotation_init(model_pcd, target, T_prev=T_prev_bias)
            n_init += 1
            clamp_count = 0
            T_anchor = T_result  # reinit gives a fresh anchor
            if fidx % 100 == 0:
                print(f"  Frame {fidx}: INIT fitness={fit:.3f}")
        else:
            ...
            # Split rotation clamp with exponential decay
            ...
            if max_angular_diff is not None and T_anchor is not None:
                ang_diff = angular_distance(T_result, T_anchor)
                if ang_diff > max_angular_diff:
                    clamp_count += 1
                    n_clamped += 1
                    alpha = ANGULAR_DECAY ** clamp_count
                    # Decayed rotation: interpolate from anchor toward zero velocity
                    if T_prev2 is not None:
                        R_delta = T_anchor[:3, :3] @ T_prev2[:3, :3].T
                        rotvec = Rot.from_matrix(R_delta).as_rotvec() * alpha
                        R_pred = Rot.from_rotvec(rotvec).as_matrix() @ T_anchor[:3, :3]
                    else:
                        R_pred = T_anchor[:3, :3]  # hold anchor orientation
                    T_result[:3, :3] = R_pred  # replace rotation only
                    # T_result[:3, 3] kept from ICP (translation grounded in point cloud)
                else:
                    clamp_count = 0
                    T_anchor = T_result  # update anchor on successful track
        ...
        if iou_gating_available and not need_init:
            ...
            if mask_gt is not None and mask_gt.any():
                ...
                iou = compute_silhouette_iou(mesh_verts, T_iou, K, scale, mask_gt, H, W)
                if iou < IOU_REINIT_THRESHOLD:
                    T_prev_bias = poses_list[fidx - 1] if fidx > 0 else None
                    T_reinit, fit_reinit = full_rotation_init(model_pcd, target, T_prev=T_prev_bias)
